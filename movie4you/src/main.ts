...
const formattedMovies: MovieTypes[] = moviesArray.map(
...
function renderMovies(movies: MovieTypes[]) {
  ...
  movies.forEach((singleMovie) => {
    ...
    year.innerHTML = singleMovie.year;

    const director = document.createElement("p");
    director.innerHTML = singleMovie.director;

    const movielength = document.createElement("p");
    movielength.innerHTML = singleMovie.movielength.toString();

    const genre = document.createElement("p");
    genre.innerHTML = singleMovie.genre.join(", "); // Array in String umwandeln

    const rating = document.createElement("p");
    rating.innerHTML = singleMovie.rating.toString();
    ...
    movieCard.append(title, year, director, movielength, genre, rating);
...
rateBtn?.addEventListener("click", sortRating);
...
function sortYearAscending() {
  const sortedMovies = [...formattedMovies];
...
////////
const genreFilter = document.querySelector(".genreFilter") as HTMLSelectElement;
console.log(genreFilter);

const filterGenreBtn = document.querySelector(".filterGenreBtn");
console.log(filterGenreBtn);
...
console.log(allPossibleGenres);

function populateGenreFilter(movies: MovieTypes[]) {
  const allGenres = new Set<string>(allPossibleGenres); // Standard-Genres vorab einfügen
  console.log(allGenres);

  movies.forEach((movie) => {
    movie.genre.forEach((g) => allGenres.add(g)); // Alle vorhandenen Genres hinzufügen
  });
...
populateGenreFilter(formattedMovies);

function filterMoviesByGenre() {
  const selectedGenre = genreFilter.value;

  if (selectedGenre === "all") {
    renderMovies(formattedMovies); // Zeige alle Filme, wenn "Alle Genres" ausgewählt ist
  } else {
...
filterGenreBtn?.addEventListener("click", filterMoviesByGenre);
